...
class NavBar extends YiiNavBar
{
    ...
    protected function renderToggleButton()
    {
        if (!CurrentUser::getUserIdentity()) {
            ...
            if (\Yii::$app->getModule('slideshow') && isset(\Yii::$app->params['slideshow']) && \Yii::$app->params['slideshow']
                === TRUE) {
                ...
                if (\Yii::$app->controller->module->id == AmosDashboard::getModuleName()) {
                    ...
                    $settingsItemsElements[]   = [
                        ...
                        'url' => ['/dashboard/manager',
                            'dashboard' => isset(\Yii::$app->controller->module) && \Yii::$app->controller->module->id != 'dashboard'
                            && !in_array(\Yii::$app->controller->module->id, $esclusioni) ? \Yii::$app->controller->module->id
                                : 'primary',
                            'slide' => 0
                        ],
            ...
            $items[] = '<li class="dropdown-header pull-right">'.Yii::t('amoscore', 'Versione').' '.((isset(\Yii::$app->params['versione']))
            ...
            $chatLink    = Html::tag('li',
                    Html::a(
                        AmosIcons::show('comments-o', [], 'dash')."<span class='badge'>".(($bulletCount > 0 ) ? $bulletCount
                            : "" )."</span>"
                        , '/messages', ['title' => \lispa\amos\chat\AmosChat::t('amoschat', 'Messaggi privati')]
